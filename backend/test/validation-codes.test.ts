...
async function appFor(nodeEnv: string): Promise<{ app: Express; stop: () => Promise<void> }> {
  vi.resetModules();
 ...
 */
const imp = (fields: string) => `mutation { importGame(input: { ${fields} }) { id } }`;
const create = (fields: string) => `mutation { createGame(input: { ${fields} }) { id } }`;

const CASES: Array<[string, string, string]> = [
  ["rawgId", imp('rawgId: "abc", title: "X"'), "rawgId must be a positive integer."],
  [
  ...
  [
    "coverUrl length",
    imp(`rawgId: "1", title: "X", coverUrl: "https://x.test/${"a".repeat(2100)}"`),
  ...
  ["releaseYear", create('title: "X", releaseYear: 1000'), "releaseYear must be a valid game release year."],
  ["title empty (hoisted validateString)", create('title: "   "'), "title must not be empty."],
];
  ...
  it("rating, yearPlayed, hoursPlayed and the duplicate review", async () => {
    ...
    try {
      ...
      const cases: Array<[string, string]> = [
        [
          review('rating: 99, content: "ok", yearPlayed: 2024, hoursPlayed: 10'),
          "rating must be between 1 and 10.",
        ],
        [
          review('rating: 7.3, content: "ok", yearPlayed: 2024, hoursPlayed: 10'),
        ...
        [
          review('rating: 8, content: "ok", yearPlayed: 2024, hoursPlayed: 99999'),
      ...
      const dup = await authedQuery(
        app,
        review('rating: 9, content: "again", yearPlayed: 2024, hoursPlayed: 10'),
        ALICE
      );
      expect(dup.errors?.[0]?.message).toBe("You have already reviewed this game.");
