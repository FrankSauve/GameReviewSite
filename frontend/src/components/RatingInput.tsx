...
export function RatingInput({
  ...
  const id = useId();
  const readout = size === "md" ? "text-display" : "text-title";
  ...
  return (
    <div className="flex items-center gap-4">
      <div className="flex-1">
        ...
        />
        <div className="flex justify-between text-meta text-content-subtle mt-0.5 select-none">
      ...
      <div className="flex items-baseline gap-1 shrink-0 w-20 justify-end">
        ...
        <span className="text-meta text-content-subtle">/ {RATING_MAX}</span>
      </div>
    </div>
  );
}
