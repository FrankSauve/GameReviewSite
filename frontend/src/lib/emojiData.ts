/**
 * Every fully-qualified emoji with its Unicode name, as the picker's search
 * index. Generated once from https://unicode.org/Public/emoji/latest/emoji-test.txt
 * (Unicode 17.0, 2025-08-04): the `fully-qualified` lines, minus the skin-tone
 * variants, with the name taken from the trailing comment.
 *
 * Committed rather than fetched: the SPA's CSP allows `connect-src 'self'`, so
 * a picker that loads its data from a CDN works in dev and dies in production.
 * Regenerate with the parser in the PR for #99, never by hand.
 */
export const EMOJI_DATA: readonly (readonly [emoji: string, name: string])[] = [
  ["😀", "grinning face"],
